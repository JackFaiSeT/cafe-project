<template>
  <v-section class="section-products" v-bind="sectionSettings">
    <block-product-list :list="products" :need-slider="dataSettings.needSlider" :loading="pending" />
  </v-section>
</template>
...
const { data: products, pending } = useAsyncData(`preview-products-${productsCode.value}`, () => {
  return productService.getProductsForPreview({ code: productsCode.value })
})
